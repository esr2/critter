...
\begin{abstract}
Semantic errors in code are stylistic errors that form the basis of poorly written or hard to read code. 
Sadly, relatively few tools have addressed semantic error checking; even fewer are customizable or 
written for C. \programName (Critique from the Terminal) fills this gap by providing a tool to check for 
user defined semantic errors in C code. It uses a SAX style of event based programming to produce 
errors as the code is being read. Administrators can use the predefined checks or create their own to 
enforce coding standards or help grade and teach ``good style'' to students.
\end{abstract}
...
highlights substantive errors in literature. With code, the compiler shows semantic errors and the user\slash 
...
many expressions are in the source code is shown in \refCode \ref{pmd-rule}.

\todo{remove the import\slash package lines?}
\begin{figure}
...
in \refCode \ref{checkstyle-rule}.
...
Complete\cite{code-complete}, C-Style\cite{standards} PMD\cite{pmd} and Checkstyle\cite{checkstyle},
...
such as \lstinline{size_t}, \lstinline{FILE}, \lstinline{pid_t}, etc. If any of these hardcoded checks pass, 
then the lexer tells the parser it has found a type name instead of an \lstinline{IDENTIFIER}.
...
location of that segment as opposed to the entire construct. For example, in \refCode \ref{grammar}, 
...
specified location (\refCode \ref{hooksQueues}). With the beginning of a function, all the elements of the 
...
routes them into a combined call in the SAX layer. This can best be seen in \refCode 
\ref{handlerTimeline}.
...
very poor style and recommend defining a variable to hold that value. Therefore \programName should
...
location to change the string value of the different error levels (currently, \lstinline{ERROR_HIGH}=``big 
...
the likes of \lstinline{printf} to deal with formatted error messages in one consolidated location.
...
\lstinline{/*----*/} etc.) and if a comment contains a string (with or without case sensitivity).
...
\section{Users}
Find out from your administrator where you should find their version of \programName. After following their installation instructions, go to your working directory \todo{mention only from working directory issue?} 
...
grade a submission, but by allowing students to pre-check their work, the submissions become easier 
...
peer reviews. This creates and automated reminder system against any code does not adhere to the 
...
grammar file). If the event just needs to be released at the correct time (i.e. it appears in a hooked 
...
\todoin{add information of how to test}
...
\addcontentsline{lot}{table}{~\ref{predefinedChecksFunctions} \hspace{4mm}Predefined Check Functions and their Relevant Handlers}
...
\item Be careful not to free things which were either never allocated (the current location passed to the Sax\slash Hook handler), might be shared between objects (filename strings) or will be freed on its own later (comment texts and locations).
\end{itemize}
