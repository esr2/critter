...
\todoin{about the currently implemented checks -- aka some are for educational purposes (the two types of isFunctionTooLong) and some are my own arbitrary style choices (checkIfElsePlacement)}
...
\todoin{explain the difference between a begin location and an end location}
...
\todoin{Hooks is not complete: common constructs are implemented but not everything (like operators)}
