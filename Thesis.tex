...
\begin{abstract}
Semantic errors cause poorly written or hard to read code. Sadly, relatively few tools have addressed 
semantic error checking and even fewer are customizable or written for C. \programName (Critique from 
the Terminal) fills this gap by providing a tool to check for administrator defined semantic errors in C 
code. It uses a SAX style of event based programming to produce errors as the code is being read. 
Administrators can use the predefined checks or create their own to enforce coding standards or help 
grade and teach ``good style'' to students.
\end{abstract}
