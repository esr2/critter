...
\section{Students}
Find out from your professor where you should find their version of \programName. After following their installation instructions, go to your working directory from the command line. Type 
``\lstinline{critTer *.c}'' (or if you only want to check one or two files, type their names instead of the 
\lstinline{*.c}). \todo{used the word critTer here!!} \programName will output any warnings about your 
code to \lstinline{stderr}.
...
Before customizing the code, please make sure you both understand how \programName works (see Chapter \ref{howItWorks} on page \pageref{howItWorks}) and have looked through the Code Conventions in Appendix \ref{conventions} on page \pageref{conventions}).
