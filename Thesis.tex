...
checks, while stored in a different file (checks.c/h), are called by these grammar rules (see \refCode 
\ref{grammar1.0}).
...
To add a check, one must first figure out how the attribute in question is represented in the grammar. 
Then one adds a function call inside the appropriate grammar rule. Each check can not only take 
advantage of the type of construction being formed (like Function 1 in Table \ref
{implementedChecks1.0}), it can also look at the location of each construction. This allows checks to 
check the length of a construction (like Function 2 in Table \ref{implementedChecks1.0}). It also allows 
checks to determine if they are still working with the same construction (like Function 3 in Table \ref
{implementedChecks1.0}). Please see \refCode \ref{checks.c1.0} for examples of a check funciton.
...
\section{Limitations}
\missingfigure{Add list of limitations of v1.0 including some of the reasons for them}
