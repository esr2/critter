...
\newcommand{\cnum}{\refstepcounter{checkCounter} \arabic{checkCounter}}
...
\begin{table}
	...
	\begin{tabular}{l l p{10cm}}
		...
		& Function & Purpose \\
		\midrule
		\cnum & ifHasBraces & Determines if the statement following an \lstinline!if! is surrounded by \{\} \\
		\cnum & isFunctionTooLong & Determines if a function's length exceeds the maximum number of lines\\
		\cnum & tooManyParameters & Determines if there a function exceeds the maximum number of input parameters \\
		\cnum & CPlusPlusComments & Throws an error on finding C++ style single-line comments \\
		\bottomrule
	\end{tabular}
	\end{center}
\end{table}
