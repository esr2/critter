...
\label{implementedChecks2.1}
	\begin{tabular}{l l p{10cm}}
		...
		& Function & Purpose \\
		\midrule
		7 & switchCasesHaveBreaks & Make sure all cases (including the \lstinline$default$) have a \lstinline$break$ statement \\
		8 & tooDeeplyNested & Throws error if a region is too deeply nested \\
		9 & useEnumNotConstOrDefine & Throws error for using \lstinline$const$ or \lstinline$#define$ to define integral constants \\
		\bottomrule
	\end{tabular}
\end{table}
