...
\newcommand{\cnum}{\refstepcounter{checkCounter} \thecheckCounter}
\usepackage{ifthen}
...
construction (like Function \tablerow{2}). It also allows checks to determine if they are still working with 
the same construction (like Function \tablerow{3}). Function \tablerow{4} is actually implemented 
through the lexer and is called on recognition of ``\lstinline{\\}''. \refCode \ref{checks.c1.0} shows 
...
\begin{table}
	...
	\begin{tabular}{l l p{10cm}}
		...
		& Function & Purpose \\
		\midrule
		\cnum & \chk[hasBraces]{ifHasBraces} & Determines if the statement following an \lstinline!if! is surrounded by \{\} \\
		\cnum & \chk{isFunctionTooLong} & Determines if a function's length exceeds the maximum number of lines\\
		\cnum & \chk{tooManyParameters} & Determines if there a function exceeds the maximum number of input parameters \\
		\cnum & \chk{CPlusPlusComments} & Throws an error on finding C++ style single-line comments \\
		\bottomrule
	\end{tabular}
	\end{center}
\end{table}
...
\label{implementedChecks1.1}
	\begin{tabular}{l l p{10cm}}
		...
		& Function & Purpose \\
		\midrule
		1 & hasBraces & Determines if the statement following an \lstinline!if!, \lstinline$for$, or \lstinline$while$ is surrounded by \lstinline!{}! \\
		\cnum & checkForComment & Throws an error if a function does not have an associated comment \\
		\cnum & switchHasDefault & Throws an error if a switch statement does not include a \lstinline$default$ case \\
	\end{tabular}
\end{table}
...
\label{implementedChecks2.1}
	\begin{tabular}{l l p{10cm}}
		...
		& Function & Purpose \\
		\midrule
		\cnum & switchCasesHaveBreaks & Make sure all cases (including the \lstinline$default$) have a \lstinline$break$ statement \\
		\cnum & tooDeeplyNested & Throws error if a region is too deeply nested \\
		\cnum & useEnumNotConstOrDefine & Throws error for using \lstinline$const$ or \lstinline$#define$ to define integral constants \\
		\bottomrule
	\end{tabular}
\end{table}
