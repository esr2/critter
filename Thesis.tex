...
\begin{table}%[h]
	...
	\begin{tabular}{ccc}
		\toprule
		Types of Errors & Tools for {\color{red}Essays} & Tools for Code \\
		\midrule
...
\marginpar{go error by error}
Anytime one writes something, from code to literature, one encounters three different types of 
...
passage appears to be poorly written within the English language. Finally when writing, one has an 
audience who highlights substantive errors --- errors within the arguments or 
premises of the essay. When writing code we have a different set of tools.  Compilers immediately alert us of 
semantic errors --- code is outside the bounds of the language. We also have tests or users that 
can highlight substantive errors -- the program is not working correctly. However, the area of semantic 
error checking --- whether code is well written within the language --- has yet to be automated fully.
...
code is easy to read and update. {\color{red}There are ways} to evaluate the user’s perspective of a 
program, most easily through automated testing. Though there are tools to evaluate the programmer’s 
perspective, {\color{red} these tools only check specific aspects of code}. Given that code quality is 
...
need to read one's code, maintaining readability is essential. Similarly, readable code is easy to 
revise and update later. In an academic environment, automated semantic error checking can 
immediately save work for professors and TAs. {\color{red} Such a program could produce a set of 
automated warnings as opposed to writing the same set of stylistic comments to many students. } 
