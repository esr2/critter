...
%project name
\usepackage{xspace}
\newcommand{\projectname}{My Project Name\xspace}
\newcommand{\programname}{a.out\xspace}
...
input.  \programname parses the input file by recognizing each code construction as part of the valid C 
grammar. While parsing the code, \programname executes any function associated with the matched 
construction -- this pairing of a construction and associated function is called a grammar rule. My 
checks, while stored in a different file (see Figure \ref{classHierarchy1.0}), are called by these 
grammar rules (see \refCode \ref{grammar1.0}).

\begin{figure}
\caption{Class Hierarchy for Version 1.0}
\label{classHierarchy1.0}
\missingfigure{Add v1.0 class hierarchy figure -- or at least diagram of how things are related to each other}
\end{figure}
...
selection_statement
	: IF `(' expression `)' statement 	{$$ = IF_SELECTION;
		ifHasBraces($5, @$);}
	| IF `(' expression `)' statement ELSE statement 
		{$$ = IF_ELSE_SELECTION, ifHasBraces($5, @$); 
		ifHasBraces($7, @$);}
...
function_definition
	: declaration_specifiers declarator declaration_list 
		compound_statement {isFunctionTooLong(@$);}
...
of construction being formed (like Function ???(1) in Table \ref{implementedChecks1.0}), it can also 
look at the location of each construction. This allows checks to check the length of a construction (like 
Function ???(2) in Table \ref{implementedChecks1.0}). It also allows checks to determine if they are 
still working with the same construction (like Function ???(3) in Table \ref{implementedChecks1.0}). 
Please see \refCode \ref{checks.c1.0} for examples of a check function.
...
void isFunctionTooLong(YYLTYPE location) {
	...
	if (location.last_line - location.first_line + 1 >=
	    MAX_FUNCTION_LENGTH) {
...
void tooManyParameters(YYLTYPE location) {
	...
	if (location.first_line != prevLoc.first_line ||
		...
		if (numParameters != -1 && numParameters >= 
			...
			char string[200];
			sprintf(string,
					``Please use less than %d function parameters,
					 you used %d'', MAX_NUM_PARAMETERS, numParameters);
...
\section{Limitations}
There are two main limitations to this version: the hassle of adding a check and macros are not handled. As described above, adding a check is nontrivial because of the interaction with the grammar file. It requires users to be reasonably familiar with not only grammar files but the C grammar itself. The second limitation is that all macros except \lstinline{#include} are completely ignored. This is far easier than the alternative which is to effectively compile and track all definitions, expressions and \lstinline{#ifndefs}. Includes are parsed only so far as to include any local file -- all standard files are ignored because they rely on types that were defined as macros. To make this system work for normal C files, the lexer has the \lstinline{size_t} type built in.
