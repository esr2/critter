...
\usepackage[pdfauthor={ErinRosenbaum},pdftitle={\projectname},pagebackref=true,pdftex]{hyperref}
...
files. These two elements combine into one larger program which takes a *.c file as input. \todoin{Fix the 
rest of Version 1 begining}



\section{Checks}

To add a check, one must first figure out how the code in question is represented in the grammar. 
Then one adds a function call inside the appropriate grammar rule. \todoin{Add rest of how a check works in 1.0} \missingfigure{Grammar file}
\missingfigure{excerpt from check.c ??}
...
\begin{table}%[h]
	...
	\begin{tabular}{l p{10cm}}
		...
		Function & Purpose \\
		\midrule
		ifHasBraces & Determines if the statement following an \lstinline!if! is surrounded by \{\} \\
		isFunctionTooLong & Determines if a function's length exceeds the maximum number of lines\\
		tooManyParameters & Determines if there a function exceeds the maximum number of input parameters \\
		CPlusPlusComments & Throws an error on finding C++ style single-line comments \\
		\bottomrule
	\end{tabular}
	\end{center}
\end{table}
