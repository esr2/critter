...
relatively few tools. \todo{fix page headers}
...
\todo{check over grammar caption}
\begin{figure} 
...
\section{Some Theory} \todo{does this need to be linked into the mini-intro?}
...
events, much like handling events in a graphical user interface''\cite{saxHomepage}\todo{do i need to 
specify that this came on /event?}. Specifically, SAX uses event handlers to parse XML files. There are 
three main handlers which are used at the beginning and end of each XML element as well as to capture 
the text in between. 
...
\todo{Should I just have all the figures come after all the text for the section?}
...
function declarations in header files.}\todo{how's this foonote?}
...
\todo{is this is a decent example for the different types of calls?}
\begin{figure}
