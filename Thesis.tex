...
\hspace{1mm} test.c:
\begin{lstlisting}[numbers=left, firstnumber=91,xleftmargin=1cm]
	for (int q = 0; q<5; q++) {
		printf("hi");
	}
\end{lstlisting}
...
\end{lstlisting}
\end{minipage} \\
\small{Here, \programName is complaining that the for loop's exit condition contains a `magic number'. The error message contains the location of the error, the warning level and then the error message.}
\end{figure}

The administrator is responsible for defining the set of semantic errors. 
