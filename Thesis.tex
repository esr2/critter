...
also look at the location of each construction. This allows checks to check the length of a construction 
(like Function \tablerow{2} in Table \ref{implementedChecks1.0}). It also allows checks to determine if 
they are still working with the same construction (like Function \tablerow{3} in Table 
\ref{implementedChecks1.0}). Please see \refCode \ref{checks.c1.0} for examples of a check function.
...
make this system work for normal C files, the lexer has the \lstinline{size_t} type built in. A minor issue 
is \programName does not follow the paths of files -- meaning that it will not run on code residing in a 
