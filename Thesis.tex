...
Instead of writing them from scratch, I found the Flex and Bison specification files online 
\cite{originalGrammar} and have modified them to add additional functionality. The only major 
...
to its previous state. The end of program occurs when there are no more files on the stack. 
\footnote{\programName starts by adding all of the given files to the stack and dynamically adding 
...
them into a combined call into the SAX layer. This can best be seen in \refCode \ref{handlerTimeline}.
...
\begin{center}
\processifversion{PRINT}{\includegraphics[scale=0.55]{ClassInteractionPrint}}
\processifversion{COLOR}{\includegraphics[scale=0.55]{ClasInteractionColor}}
\end{center}
\end{figure}
